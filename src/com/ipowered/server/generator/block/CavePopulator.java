package com.ipowered.server.generator.block;
...
public class CavePopulator extends BlockPopulator {
	...
	private static class FinishSnakeTask implements Runnable {
		...
		public void run() {
			...
			for (Location loc : snake) {
				loc.getWorld().unloadChunkRequest(loc.getBlockX() / 16,
						loc.getBlockZ() / 16);
			}
		}
	}
		...
		if (random.nextInt(100) < 10) {
			...
			new Thread(new Runnable() {
				...
				public void run() {
					...
					// Bukkit.getServer().getScheduler().scheduleSyncDelayedTask(GenPlugin.instance,
					// new FinishSnake(world, snake));
	...
	private static Set<Location> startSnake(World world, Random random,
						...
						for (int z = -radius; z <= radius; z++) {
							if (x * x + y * y + z * z <= radius2 && y >= 0
								...
								if (world.getBlockTypeIdAt(blockX + x, blockY
									...
									snakeBlocks.add(new Location(world, blockX
											+ x, blockY + y, blockZ + z));
								}
							}
						}
